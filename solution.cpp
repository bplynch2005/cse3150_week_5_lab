...
std::vector<int> compress_row(const std::vector<int>& row) {
	...
	std::copy_if(row.begin(), row.end(), back_inserter(compressed), [](int n){return n != 0;});
...
void print_board(const vector<vector<int>>& board) {
	...
	std::cout << compute_score(board) << std::endl;
...
bool move_left(std::vector<std::vector<int>>& board) {
	...
	for(auto &row : board){
		...
		compressed = merge_row(compressed);
		compressed = compress_row(compressed);
		if(row != compressed){
			moved = true;
			row = compressed;
		}
...
bool move_up(std::vector<std::vector<int>>& board) {
	...
	for(int c = 0; c < 4; ++c){
		...
		std::vector<int> prevColumn = column;
